...
public interface LobbyRepository extends JpaRepository<Lobby, Integer> {
	...
	@Query("SELECT pd.lobby FROM PlayerDetails pd WHERE pd.lobby.available AND pd.isLobbyOwner AND pd.username = :username")
	Set<Lobby> getOwnedLobbies(@Param("username") String username);

	@Query("SELECT pd.lobby FROM PlayerDetails pd WHERE pd.lobby.available AND pd.isLobbyOwner AND pd.lobby.privacyStatus != 'PRIVATE' AND pd.username IN :friendsUsernames")
	Set<Lobby> getLobbiesOwnedByFriends(@Param("friendsUsernames") List<String> friendsUsernames);
	
	@Query("SELECT l FROM Lobby l WHERE l.available AND l.privacyStatus = 'PUBLIC'")
